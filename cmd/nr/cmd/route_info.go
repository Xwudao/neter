...
func runRouteInfoGenTS(cmd *cobra.Command) error {
	...
	if singleFile {
		...
		if err := utils.WriteFileAtomic(output, []byte(generated), 0o644); err != nil {
...
func runRouteInfoGenTSDir(projectDir, outputDir string, routes *route_info.ProjectRoutes, check bool) error {
	...
	if check {
		...
		if extra, err := staleGeneratedTypeScriptFiles(outputDir, files); err != nil {
			return err
		} else {
	...
	for _, name := range names {
		if err := utils.WriteFileAtomic(filepath.Join(outputDir, name), []byte(files[name]), 0o644); err != nil {
			return fmt.Errorf("write generated TypeScript file %s: %w", name, err)
		}
	}
	if err := removeStaleGeneratedTypeScriptFiles(outputDir, files); err != nil {
		return err
	}
...
const generatedTypeScriptHeader = "// Code generated by nr route-info gen-ts. DO NOT EDIT."

// staleGeneratedTypeScriptFiles only considers files that bear nr's generated
// header, so cleanup never removes a hand-written client helper.
func staleGeneratedTypeScriptFiles(outputDir string, expected map[string]string) ([]string, error) {
	entries, err := os.ReadDir(outputDir)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, nil
		}
		return nil, fmt.Errorf("read generated TypeScript directory: %w", err)
	}
	var stale []string
	for _, entry := range entries {
		if entry.IsDir() || !strings.HasSuffix(entry.Name(), ".gen.ts") {
			continue
		}
		...
		if err != nil {
			return nil, fmt.Errorf("read generated TypeScript file %s: %w", entry.Name(), err)
		}
		if strings.HasPrefix(string(content), generatedTypeScriptHeader) {
...
func removeStaleGeneratedTypeScriptFiles(outputDir string, expected map[string]string) error {
	stale, err := staleGeneratedTypeScriptFiles(outputDir, expected)
	if err != nil {
		return err
	}
	for _, name := range stale {
		if err := os.Remove(filepath.Join(outputDir, name)); err != nil {
			return fmt.Errorf("remove stale generated TypeScript file %s: %w", name, err)
		}
	}
	return nil
}
